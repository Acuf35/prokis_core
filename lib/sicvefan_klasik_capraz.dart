...
class SicVeFanKlasikCapraz extends StatefulWidget {
  List<Map> gelenDBveri;
  SicVeFanKlasikCapraz(List<Map> dbVeriler) {
    gelenDBveri = dbVeriler;
  }

  @override
  State<StatefulWidget> createState() {
    return SicVeFanKlasikCaprazState(gelenDBveri);
  }
}

class SicVeFanKlasikCaprazState extends State<SicVeFanKlasikCapraz> {
...
//++++++++++++++++++++++++++CONSTRUCTER METHOD+++++++++++++++++++++++++++++++
  SicVeFanKlasikCaprazState(List<Map> dbVeri) {
                                ...
                                children: <Widget>[
                                  ...
                                  Expanded(
                                      ...
                                      child: charts.BarChart(
                                        ...
                                        _grafikDataKlasikCapraz(
                                            double.parse(setSicA),
                                            double.parse(dogalBolgeB),double.parse(capHavFarkC),dilSecimi,fanSet),
                                        ...
                                        primaryMeasureAxis:
                                          ...
                                          tickProviderSpec: new charts
                                            ...
                                            <charts.TickSpec<num>>[
                                              ...
                                              charts.TickSpec<num>(double.parse(
                                                  setSicA) +
                                              double.parse(dogalBolgeB)+double.parse(capHavFarkC)),
                                              charts.TickSpec<num>(
                                                      fanSet.reduce(max)),
...
//++++++++++++++++++++++++++METOTLAR+++++++++++++++++++++++++++++++
  ...
  Future _degergiris2X1(int onlar, birler, ondalik, index, double oran,
    ...
    await showDialog(
      ...
      if (veriGonderilsinMi) {
        ...
        _veriGonder(
            "3*$setSicA*$dogalBolgeB*$capHavFarkC*$gonderilecekFanIndex*$gonderilecekFanSet");
        print(capHavFarkC);
      }
  ...
  static List<charts.Series<GrafikSicaklikCizelgesi, String>> _grafikDataKlasikCapraz(
      double setSic, dogBol, capBol, String dil, List<double> fanSet) {
  ...
  _takipEt() async {
    try {
      ...
      await Socket.connect('192.168.1.110', 2236).then((socket) {
        socket.add(utf8.encode('3*$fanAdet'));
        ...
        socket.listen(
          (List<int> event) {
            ...
            if (gelenMesaj != "") {
              ...
              capHavFarkC = degerler[2];

              for (int i = 1; i <= 60; i++) {
                fanSet[i] = double.parse(degerler[i + 2]);
              }
              print(fanSet
                              .reduce(max) -
                          (double.parse(setSicA) +
