...
//++++++++++++++++++++++++++SCAFFOLD+++++++++++++++++++++++++++++++
    return Scaffold(
        appBar: Metotlar().appBar(
            dilSecimi, context, oran, 'tv475', baglantiDurum, alarmDurum),
        body: Column(
          children: <Widget>[
            //Saat ve Tarih
            Row(
...
//++++++++++++++++++++++++++METOTLAR+++++++++++++++++++++++++++++++
  ...
  takipEtVeriIsleme(String gelenMesaj, String komut) {
    ...
    if (komut.split("*")[0] == "24") {
      for (int i = 1; i <= int.parse(klepeAdet); i++) {
        ...
        klpManKp[i] =
            degerler[i - 1 + int.parse(klepeAdet)] == "True" ? true : false;
        baglantiKlp[i] = false;
      }
      int x = degerler.length;
      klpHareketSuresi = degerler[x - 2];
      alarmDurum = degerler[x - 1];
    }
  ...
  Widget _unsurOtoManWidget(String baslik, String imagePath, double oran,
      int index, DBProkis dbProkis) {
    return Visibility(
      visible: index <= int.parse(klepeAdet) || index > 5,
      child: Expanded(
        child: Visibility(
          visible: index <= int.parse(klepeAdet),
          child: Row(
            children: <Widget>[
              Spacer(),
              Expanded(
                              ...
                              child: RawMaterialButton(
                                onPressed: () {
                                  ...
                                  Timer.periodic(Duration(seconds: 1), (timer) {
                                    ...
                                    if (!baglantiKlp[index] &&
                                        yazmaSonrasiGecikmeSayaciKLP[index] >
                                            1) {
                                      baglantiKlp[index] = true;

                                      Metotlar()
                                          .takipEt('24*$klepeAdet', 2236)
                                          .then((veri) {
                                        if (veri.split("*")[0] == "error") {
                                          baglanti = false;
                                          baglantiDurum = Metotlar()
                                  ...
                                  _manKontrolKLPE(oran, index, dbProkis)
                                      .then((value) {
                                  ...
                                  Column(
                                    children: <Widget>[
                                      ...
                                      RaisedButton(
                                        onPressed: () {
                                          _index = index;
                                          _onlar = int.parse(klpHareketSuresi
                                                      .split(".")[0]) <
                                                  10
                                              ? 0
                                              : (int.parse(klpHareketSuresi
                                                      .split(".")[0]) ~/
                                                  10);
                                          _birler = int.parse(klpHareketSuresi
                                                  .split(".")[0]) %
                                              10;
                                          _ondalik = int.parse(
                                              klpHareketSuresi.split(".")[1]);
                                  ...
                                  Row(
                                    children: <Widget>[
                                      ...
                                      bottomDrawerManUnsur(index, "tv473",
                                          klpManAc[index], oran, dbProkis),
                                      Spacer(),
                                      bottomDrawerManUnsur(index + 10, "tv474",
                                          klpManKp[index], oran, dbProkis),
                                      Spacer(
                                        flex: 3,
                                      ),
  ...
  Widget bottomDrawerManUnsur(int index, String isim, bool otoManDurum,
      double oran, DBProkis dbProkis) {
    return Expanded(
      flex: 10,
      child: RawMaterialButton(
