...
import 'package:prokis/genel/sabitVeriler.dart';
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints viewportConstraints) {
          ...
          TextEditingController tec1 =
              new TextEditingController(text: provider.getKumesIsmi);
          TextEditingController tec2 =
              new TextEditingController(text: provider.getSifreAna);
          TextEditingController tec3 =
              new TextEditingController(text: provider.getSifreTekrar);

          _textFieldCursorPosition(tec1, provider.getKumesIsmi);
          _textFieldCursorPosition(tec2, provider.getSifreAna);
          _textFieldCursorPosition(tec3, provider.getSifreTekrar);
                                              ...
                                              child: RawMaterialButton(
                                                onPressed: () async {
                                                  if (!provider
                                                    ...
                                                    Metotlar().handle('1*2*${provider.getKumesTuruIndex.toString()}*${provider.getKumesNo}*${provider.getKumesIsmi}*${provider.getSifreAna}',
                                                            context,
                                                            2233,
                                                            "toast21",
                                                            dilSecimi).then((value) => print("aktarılan deger:$value"));
/*
                                                    String deneme= await Metotlar()
                                                        .veriGonder(
                                                            '1*2*${provider.getKumesTuruIndex.toString()}*${provider.getKumesNo}*${provider.getKumesIsmi}*${provider.getSifreAna}',
                                                            context,
                                                            2233,
...
class TemelAyarlarProvider with ChangeNotifier {
  ...
  TemelAyarlarProvider(this.context, this.dbProkis) {
    _dilSecimi = dbProkis.dbVeriGetir(1, 1);

    switch (dbProkis.dbVeriGetir(3, 1)) {
      case "1":
    ...
    _kumesTuruIndex = int.parse(dbProkis.dbVeriGetir(3, 1));
    _kumesNo =
        dbProkis.dbVeriGetir(3, 2) == "yok" ? "1" : dbProkis.dbVeriGetir(3, 2);
    _kumesIsmi =
        dbProkis.dbVeriGetir(3, 3) == "yok" ? "" : dbProkis.dbVeriGetir(3, 3);
    _sifreAna =
        dbProkis.dbVeriGetir(3, 4) == "yok" ? "" : dbProkis.dbVeriGetir(3, 4);
    _sifreAnaGecici = _sifreAna;
    _sifreTekrar = _sifreAna;
    _sifreTekrarGecici = _sifreAna;
    _sifreOnaylandi = dbProkis.dbVeriGetir(3, 4) == "yok" ? false : true;
    _sifreUyusma =
